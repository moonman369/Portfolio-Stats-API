...
function assertSummaryConstraints(document) {
  ...
  // The Atlas autoEmbed index vectorizes the configured text field
  // (VECTOR_TEXT_FIELD_PATH), so summary_for_embedding only matters for
  // retrieval when content_full is empty. When content_full is provided the
  // summary is a display-only field, so we don't constrain its shape.
